...
public class PlayerCombat : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (Time.time >= nextAttackTime && (Input.GetButtonDown("Attack") || Input.GetKeyDown(KeyCode.Mouse0)))
            {   
    ...
    void Attack()
    {
        ...
        foreach (Collider2D enemy in hitEnemies)
        {
            if (enemy.name == "Skeleton" &&
                !enemy.GetComponent<Animator>().GetCurrentAnimatorStateInfo(0).IsName("Skeleton_TakeDamage"))
            {
